...
/* Shopping UI state */
let manualSelected = new Set(JSON.parse(localStorage.getItem('manualSelected') || '[]')); // meal indices selected on Select Meals page
...
function renderManualList() {
  ...
  meals.forEach((m, idx) => {
    ...
    btn.addEventListener('click', () => {
      ...
      // update UI
      renderManualList();
      // update shopping list immediately
      renderShoppingList();
...
function refreshShoppingList() {
  // Prune clearedShopping to current combined keys and re-render
  renderShoppingList();
}
...
window.addEventListener('load', () => {
  // Ensure planSnapshot is initialized on load
  if (!planSnapshot) {
