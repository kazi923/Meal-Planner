...
function save() {
  localStorage.setItem('meals', JSON.stringify(meals));
  localStorage.setItem('plan', JSON.stringify(plan));
  const newSnap = computePlanSnapshot();
  if (newSnap !== planSnapshot) {
    planSnapshot = newSnap;
...
function renderManualList() {
  const c = manualList; c.innerHTML = "";
  meals.forEach((m,i) => {
...
function renderShoppingList() {
  ...
  const l = shoppingItems; l.innerHTML = "";
  seen.forEach((pretty,key) => {
    ...
    d.appendChild(left); l.appendChild(d);
  });
}
...
function refreshShoppingList() { renderShoppingList(); }

function exportShoppingList() {
  const items = [...document.querySelectorAll('#shoppingItems span')].map(e => e.textContent);
